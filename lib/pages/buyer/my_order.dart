...
import 'package:bhattimandu/pages/buyer/add-review.dart';
...
class _MyOrderState extends State<MyOrder> with TickerProviderStateMixin {
  ...
  Future<void> fetchOrder() async {
    try {
      ...
      final fetchedActiveOrders = await MyCartController().getCustOrderItems(
        uuid: user!.uid,
        context: context,
        status: 'pending',
      );
      final fetchedCompletedOrders = await MyCartController().getCustOrderItems(
        uuid: user.uid,
        context: context,
        status: 'completed',
      );
        ...
        return GestureDetector(
          onTap:status == "Completed Orders" ? (){
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => AddReview(productId:order['liquorId']),
              ),
            );
          }:(){},
          child: Card(
